...
| B-09 | history_guard, role_guard | A pull request merged through the REST API with `curl` (or another HTTP client), such as `curl -X PUT -H "Authorization: …" https://api.github.com/repos/<owner>/<repo>/pulls/<N>/merge`, which neither the `gh pr merge` rule nor B-03's `gh api` check sees. | open | `test_b09_pr_merge_through_curl` |
